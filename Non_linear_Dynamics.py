...
out_vid = cv2.VideoWriter('non_linear_dynamics.avi',cv2.VideoWriter_fourcc(*'DIVX'), 15, (1000,1000))
...
for i in range(500):
    ...
    Environment = np.ones((1000,1000,3), np.uint8)*255


    if(state[2] <-270):control =1
    if(state[2] >90):control = -1
    curr_v = v + np.random.normal(0, 2, 1)[0]
    curr_w = ( w + np.random.normal(0, 0.25, 1)[0] ) * control
    ...
    if(ekf):
        ...
        DATA.append([v,w*control,*(state.copy())])
